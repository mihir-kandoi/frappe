...
import frappe
from frappe.desk.link_title import MAX_REPORT_TITLES, get_report_link_titles
...
data = columns, result
	...
	def test_link_titles_are_dropped_past_the_cap(self):
		"""Past the cap every row keeps its document name, rather than titles on an arbitrary subset."""
		self.enable_link_titles("User")
		columns = [{"fieldname": "allocated_to", "fieldtype": "Link", "options": "User"}]
		rows = [{"allocated_to": "Administrator"}]
		self.assertTrue(get_report_link_titles(columns, rows))

		rows += [{"allocated_to": f"unknown-{index}"} for index in range(MAX_REPORT_TITLES)]
		self.assertEqual(get_report_link_titles(columns, rows), {})
